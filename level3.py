...
def main(userID):
    ...
    # music/back pause settings and timer
        ...
        if boxx != None and boxy != None:
            ...
            if not revealedBoxes[boxx][boxy] and mouseClicked:
                ...
                if firstSelection == None: # the current box was the first box clicked
                    ...
                    #if (icon1shape != icon2shape  or icon1color != icon2color) and (icon1shape != icon3shape  or icon1color != icon3color) :
                    
                    if len(list(set([icon1shape,icon2shape,icon3shape])))>1 or len(list(set([icon1color,icon2color,icon3color])))>1:
                        # Icons don't match. Re-cover up both selections.
                        pygame.time.wait(1000) # 1000 milliseconds = 1 sec
                        coverBoxesAnimation(mainBoard, [(firstSelection[0], firstSelection[1]),secondSelection, (boxx, boxy)])
                        ...
                        dfScore=pd.DataFrame({"Level":[3],"UserID":[userID],"TimeTaken":[timeTaken],"Time":[timeTakenStr]})
...
def getRandomizedBoard(num=3):
    ...
    numIconsUsed = int(BOARDWIDTH * BOARDHEIGHT / num) # calculate how many icons are needed
    icons = icons[:numIconsUsed] * num # make two of each
